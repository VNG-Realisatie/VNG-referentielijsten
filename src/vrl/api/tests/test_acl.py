...
class CommunicatieKanaalTests(APITestCase):
    ...
    def test_update_forbidden(self):
        comm_kanaal = CommunicatieKanaal.objects.create(
            naam="telefoon", omschrijving="banana phone"
        )
        url = reverse("communicatiekanaal-detail", kwargs={"uuid": comm_kanaal.uuid})

        response = self.client.put(url, {"naam": "foo", "omschrijving": "bar"})

        self.assertGreater(response.status_code, 400)
        self.assertLess(response.status_code, 500)

        response = self.client.patch(url, {"naam": "foo", "omschrijving": "bar"})
