...
class LoadDataFromExcelTest(TestCase):
    ...
    def test_parse_duration_direct(self):
        ...
        self.assertEqual(
            parse_duration('Na kennisgeving van Minister dat de grond tot weigering of intrekking is vervallen terstond vernietigen'),
            relativedelta(days=0)
        )
    ...
    def test_parse_duration_integer_period(self):
        ...
        self.assertEqual(
            parse_duration('4 weken'),
            relativedelta(weeks=4)
        )

        self.assertEqual(
            parse_duration('6 maanden'),
