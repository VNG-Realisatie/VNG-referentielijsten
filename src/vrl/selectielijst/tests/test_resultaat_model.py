...
class ResultaatTests(TestCase):
    ...
    def test_volledig_nummer(self):
        generiek = ResultaatFactory.create(
            proces_type__nummer=4,
            nummer=2
        )
        specifiek = ResultaatFactory.create(
            proces_type=generiek.proces_type,
            generiek_resultaat=generiek,
            nummer=7
        )

        self.assertEqual(
            generiek.volledig_nummer,
            '4.2'
        )
        self.assertEqual(
            specifiek.volledig_nummer,
            '4.2.7'
        )
